...
def _chart(fig, height: int = 400) -> None:
    ...
    render_plotly_chart(st, fig, height=height)
...
_slider_max = slider_max_year(all_years, current_year=_current_year)
...
def page_player_explorer() -> None:
    ...
    has_name_collision = has_duplicate_names(filt)
    id_col = player_id_columns_for_collisions(filt)
...
def page_team_profile() -> None:
    ...
    if players is not None:
        ...
        if not roster.empty:
            roster_id = player_id_columns_for_collisions(roster)
