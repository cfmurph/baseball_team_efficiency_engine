...
from src.baseball_analytics import dashboard_utils
...
_PLOTLY_LAYOUT = dashboard_utils.PLOTLY_LAYOUT
...
def _apply_layout(fig) -> None:
    ...
    dashboard_utils.apply_plotly_layout(fig)


def _chart(fig, height: int = 400) -> None:
    """Apply dark layout and render a Plotly chart."""
    dashboard_utils.render_plotly_chart(fig, st.plotly_chart, height=height)
...
_slider_max = dashboard_utils.slider_max_year(all_years, _current_year)
...
def page_player_explorer() -> None:
    ...
    has_name_collision = dashboard_utils.has_same_name_players(filt)
    id_col = dashboard_utils.player_id_columns_for_collisions(filt)
...
def page_team_profile() -> None:
    ...
    if players is not None:
        ...
        if not roster.empty:
            roster_id = dashboard_utils.player_id_columns_for_collisions(roster)
