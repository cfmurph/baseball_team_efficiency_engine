...
def scale_payroll(df: pd.DataFrame) -> pd.DataFrame:
    """Convert raw dollar columns to millions for dashboard display."""
    df = df.copy()
...
def slider_max(years: Iterable[int], current_year: int | None = None) -> int:
    """Return a safe dashboard slider maximum for empty, stale, or future-dated data."""
...
def render_plotly_chart(fig: Any, streamlit_module: Any, height: int = 400) -> None:
    """Apply dashboard Plotly layout and delegate exactly once to Streamlit."""
...
def has_name_collision(df: pd.DataFrame, name_col: str = "name_full") -> bool:
    """Return True when the current table view contains multiple rows for a player name."""
...
def player_id_columns_for_name_collisions(
    ...
    """Expose player_id only when it is needed to distinguish same-name players."""
