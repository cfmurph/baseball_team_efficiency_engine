...
def scale_payroll(df: pd.DataFrame) -> pd.DataFrame:
    """Convert raw dollar and ratio columns into dashboard display units."""
    df = df.copy()
...
def slider_max(years: Iterable[int], current_year: int | None = None) -> int:
    """Return a safe max year for dashboard sliders even when artifacts are empty."""
    year_list = sorted(int(year) for year in years if pd.notna(year))
...
def apply_plotly_layout(fig: Any) -> None:
    """Apply the dashboard's dark Plotly theme to a figure."""
...
def render_plotly_chart(
    ...
    *,
    ...
    """Apply dashboard layout then delegate rendering to Streamlit."""
