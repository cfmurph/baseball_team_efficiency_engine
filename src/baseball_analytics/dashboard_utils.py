...
def slider_max(years: Iterable[int], current_year: int) -> int:
    """Return a slider upper bound that handles empty data and future datasets."""
...
def render_plotly_chart(fig: Any, streamlit_module: Any, height: int = 400) -> None:
    """Apply shared chart styling before delegating rendering to Streamlit."""
    apply_plotly_layout(fig)
    fig.update_layout(height=height)
    streamlit_module.plotly_chart(fig, use_container_width=True)


def has_player_name_collision(df: pd.DataFrame, name_col: str = "name_full") -> bool:
    """Return True when the current view contains multiple players with the same name."""
...
def player_id_columns(df: pd.DataFrame, name_col: str = "name_full", id_col: str = "player_id") -> list[str]:
    ...
    return [id_col] if has_player_name_collision(df, name_col) and id_col in df.columns else []
