...
| Integration | `integration` | Warehouse / metrics / storage / fantasy emitter / MLB Stats API ingest with fixtures or `file://` backends. Nightly `PIPELINE_STEPS` contract (`pull_war` after `pull_sources`, `pull_mlb_stats` after `pull_war`). |
...
No layer talks to live Baseball-Reference, MLB Stats API, or object storage. E2E uses committed fixtures and in-process AppTest only.
...
| **Integration tests** | `pytest -m integration` | Nightly pipeline contract (`tests/test_run_nightly.py`) |
