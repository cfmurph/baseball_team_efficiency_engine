...
If `baseUrl` is empty the client prefers `http://127.0.0.1:8000` when that process answers `/v1/health`, otherwise fixture cards (`fantasy/stub_cards.jsonl`). Types match the OpenAPI so setting `NEXT_PUBLIC_API_URL` is the only production swap. Prior-year cards still ship; live 2026 is #131, not a client gate.
