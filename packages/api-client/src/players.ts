...
function parsePitchingSeason(value: unknown): PitchingSeason | null {
  ...
  return withPitchingIdentities({
    ...
    so: num(raw.pitching_so ?? raw.so),
    bb: num(raw.pitching_bb ?? raw.bb),
    er: num(raw.er),
    era: num(raw.era),
    whip: num(raw.whip),
    fip: num(raw.fip),
    war: num(raw.war),
    war_source: text(raw.war_source),
    h: num(raw.pitching_hits ?? raw.h ?? raw.hits),
    hr: num(raw.pitching_hr ?? raw.hr),
    r: num(raw.pitching_r ?? raw.r ?? raw.runs),
    uer: num(raw.uer),
    cg: num(raw.cg),
    sho: num(raw.sho),
    hld: num(raw.hld),
    ...
    ir: num(raw.ir),
    bf: num(raw.bf),
    go: num(raw.pitching_go ?? raw.go),
    ao: num(raw.pitching_ao ?? raw.ao),
    go_ao: num(raw.pitching_go_ao ?? raw.go_ao),
    wpct: num(raw.wpct),
    k9: num(raw.k9),
    bb9: num(raw.bb9),
    h9: num(raw.h9),
