...
- **BenchOrStart copy + 2026:** Use `fantasy/copy.py` as-is unless Cole edits it. Do not invent marketing copy. Live 2026 rows are not a ship gate for `apps/web` — if `/v1` is prior-only, show `PRIOR_SEASON_BANNER` and ship those cards. Keyed 2026 publish stays on #131.
