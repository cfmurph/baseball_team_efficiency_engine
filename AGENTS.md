...
- **BenchOrStart copy + 2026:** Port `fantasy/copy.py` VERBATIM onto Next.js (product name, headline, CTA, tabs, footer, empty states). Cole owns that file — do not rewrite. Live 2026 rows are not a ship gate for `apps/web` — if `/v1` is prior-only, show `PRIOR_SEASON_BANNER` and ship those cards. Keyed 2026 publish stays on #131.
