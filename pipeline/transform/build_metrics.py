...
SELECT
    ...
    SUM(p.hr)                           AS hr,
    SUM(p.bb)                           AS bb,
    SUM(CASE WHEN p.pa > 0 THEN p.woba * p.pa END)
        / NULLIF(SUM(CASE WHEN p.pa > 0 THEN p.pa END), 0) AS woba,

    SUM(p.batting_war)                  AS batting_war,
    SUM(p.ip)                           AS ip,
    SUM(CASE WHEN p.ip > 0 THEN p.fip * p.ip END)
        / NULLIF(SUM(CASE WHEN p.ip > 0 THEN p.ip END), 0) AS fip,
    SUM(CASE WHEN p.ip > 0 THEN p.era * p.ip END)
        / NULLIF(SUM(CASE WHEN p.ip > 0 THEN p.ip END), 0) AS era,
