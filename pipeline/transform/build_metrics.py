...
SELECT
    ...
    SUM(CASE WHEN p.pa > 0 AND p.woba IS NOT NULL THEN p.woba * p.pa END)
        / NULLIF(SUM(CASE WHEN p.pa > 0 AND p.woba IS NOT NULL THEN p.pa ELSE 0 END), 0)
                                        AS woba,

    SUM(p.batting_war)                  AS batting_war,
    SUM(p.ip)                           AS ip,
    SUM(CASE WHEN p.ip > 0 AND p.fip IS NOT NULL THEN p.fip * p.ip END)
        / NULLIF(SUM(CASE WHEN p.ip > 0 AND p.fip IS NOT NULL THEN p.ip ELSE 0 END), 0)
                                        AS fip,
    SUM(CASE WHEN p.ip > 0 AND p.era IS NOT NULL THEN p.era * p.ip END)
        / NULLIF(SUM(CASE WHEN p.ip > 0 AND p.era IS NOT NULL THEN p.ip ELSE 0 END), 0)
                                        AS era,
