...
SELECT
    ...
    SUM(p.bb)                           AS bb,
    CASE
        WHEN SUM(CASE WHEN p.pa > 0 THEN p.pa ELSE 0 END) > 0
        THEN SUM(CASE WHEN p.pa > 0 THEN p.woba * p.pa ELSE 0 END)
             / SUM(CASE WHEN p.pa > 0 THEN p.pa ELSE 0 END)
    END                                 AS woba,

    SUM(p.batting_war)                  AS batting_war,
    SUM(p.ip)                           AS ip,
    CASE
        WHEN SUM(CASE WHEN p.ip > 0 THEN p.ip ELSE 0 END) > 0
        THEN SUM(CASE WHEN p.ip > 0 THEN p.fip * p.ip ELSE 0 END)
             / SUM(CASE WHEN p.ip > 0 THEN p.ip ELSE 0 END)
    END                                 AS fip,
    CASE
        WHEN SUM(CASE WHEN p.ip > 0 THEN p.ip ELSE 0 END) > 0
        THEN SUM(CASE WHEN p.ip > 0 THEN p.era * p.ip ELSE 0 END)
             / SUM(CASE WHEN p.ip > 0 THEN p.ip ELSE 0 END)
    END                                 AS era,
...
LEFT JOIN dim_team t ON t.team_key = p.team_id || '_' || CAST(p.season_key AS VARCHAR)
