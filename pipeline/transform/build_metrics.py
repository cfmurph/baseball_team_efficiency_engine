...
SELECT
    ...
    SUM(p.hr)                           AS hr,
    SUM(p.bb)                           AS bb,
    SUM(p.woba * p.pa) FILTER (WHERE p.pa > 0 AND p.woba IS NOT NULL)
        / NULLIF(SUM(p.pa) FILTER (WHERE p.pa > 0 AND p.woba IS NOT NULL), 0) AS woba,

    SUM(p.batting_war)                  AS batting_war,
    SUM(p.ip)                           AS ip,
    SUM(p.fip * p.ip) FILTER (WHERE p.ip > 0 AND p.fip IS NOT NULL)
        / NULLIF(SUM(p.ip) FILTER (WHERE p.ip > 0 AND p.fip IS NOT NULL), 0) AS fip,
    SUM(p.era * p.ip) FILTER (WHERE p.ip > 0 AND p.era IS NOT NULL)
        / NULLIF(SUM(p.ip) FILTER (WHERE p.ip > 0 AND p.era IS NOT NULL), 0) AS era,
