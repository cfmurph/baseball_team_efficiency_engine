...
SELECT
    ...
    SUM(p.hr)                           AS hr,
    SUM(p.bb)                           AS bb,
    SUM(CASE WHEN p.pa > 0 THEN p.pa * p.woba END)
        / NULLIF(SUM(CASE WHEN p.pa > 0 AND p.woba IS NOT NULL THEN p.pa END), 0) AS woba,

    SUM(p.batting_war)                  AS batting_war,
    SUM(p.ip)                           AS ip,
    SUM(CASE WHEN p.ip > 0 THEN p.ip * p.fip END)
        / NULLIF(SUM(CASE WHEN p.ip > 0 AND p.fip IS NOT NULL THEN p.ip END), 0) AS fip,
    SUM(CASE WHEN p.ip > 0 THEN p.ip * p.era END)
        / NULLIF(SUM(CASE WHEN p.ip > 0 AND p.era IS NOT NULL THEN p.ip END), 0) AS era,
...
LEFT JOIN dim_team t ON t.team_key = p.team_id || '_' || p.season_key
