...
SELECT
    ...
    SUM(p.bb)                           AS bb,
    CASE
        WHEN SUM(CASE WHEN p.pa > 0 AND p.woba IS NOT NULL THEN p.pa ELSE 0 END) > 0
        THEN
            SUM(CASE WHEN p.pa > 0 AND p.woba IS NOT NULL THEN p.woba * p.pa ELSE 0 END)
            / SUM(CASE WHEN p.pa > 0 AND p.woba IS NOT NULL THEN p.pa ELSE 0 END)
    END                                 AS woba,

    SUM(p.batting_war)                  AS batting_war,
    SUM(p.ip)                           AS ip,
    CASE
        WHEN SUM(CASE WHEN p.ip > 0 AND p.fip IS NOT NULL THEN p.ip ELSE 0 END) > 0
        THEN
            SUM(CASE WHEN p.ip > 0 AND p.fip IS NOT NULL THEN p.fip * p.ip ELSE 0 END)
            / SUM(CASE WHEN p.ip > 0 AND p.fip IS NOT NULL THEN p.ip ELSE 0 END)
    END                                 AS fip,
    CASE
        WHEN SUM(CASE WHEN p.ip > 0 AND p.era IS NOT NULL THEN p.ip ELSE 0 END) > 0
        THEN
            SUM(CASE WHEN p.ip > 0 AND p.era IS NOT NULL THEN p.era * p.ip ELSE 0 END)
            / SUM(CASE WHEN p.ip > 0 AND p.era IS NOT NULL THEN p.ip ELSE 0 END)
    END                                 AS era,
